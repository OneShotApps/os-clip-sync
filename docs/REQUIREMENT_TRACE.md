...
| Adaptive visible-history refresh on Android, iOS, and macOS | Shared `HistoryRefreshScheduler`, change-aware history publication, isolated spinning Refresh icons, target lifecycle/window visibility hooks, manual restart controls, and target-specific paused indicators. |
