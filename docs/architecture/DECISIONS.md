...
- Multi-item deletion is coordinated by each client through the existing owner-scoped item-delete route. This keeps the POC API small; successfully deleted items disappear immediately, while any items not deleted after a request failure remain selected for retry.
- Each installed client registers its stable client UID, platform, and operating-system-reported name after authentication. Device identity is unique within the authenticated account, so the same installation can sign into a different account without colliding with or taking over its prior account's device record. An optional account-assigned name overrides the reported name without changing device identity.
